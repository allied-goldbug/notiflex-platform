...
2. 저장소 문서를 파악한 내용에 맞춰 갱신한다. 후보 (존재하는 것만 처리):
   - `JOURNEY.md` — 진행 현황, 도구 선택, 현재 버전, 리소스 상태
     ...
     **현재 버전**: 이번 장에서 설치하거나 버전이 바뀐 컴포넌트는 추측하지 않고 클러스터에서 직접 조회해 채운다. 비워두지 않는다.
     - Notiflex 이미지: `kubectl --context gke_project-d64f9b5c-20c8-4906-95b_asia-northeast3-a_notiflex-cluster get rollout notiflex-api -n notiflex -o jsonpath='{.spec.template.spec.containers[0].image}'`
     - ArgoCD: `kubectl --context gke_project-d64f9b5c-20c8-4906-95b_asia-northeast3-a_notiflex-cluster get deploy argocd-server -n argocd -o jsonpath='{.spec.template.spec.containers[0].image}'`
     - Kafka: `kubectl --context gke_project-d64f9b5c-20c8-4906-95b_asia-northeast3-a_notiflex-cluster get kafka -n kafka -o jsonpath='{.items[0].spec.kafka.version}'`
     - 그 외 도구: `kubectl --context gke_project-d64f9b5c-20c8-4906-95b_asia-northeast3-a_notiflex-cluster get pod -n <namespace> -l <label> -o jsonpath='{.items[0].spec.containers[0].image}'`
     **현재 리소스**: 노드풀이 추가·변경됐으면 `kubectl --context gke_project-d64f9b5c-20c8-4906-95b_asia-northeast3-a_notiflex-cluster get nodes -L cloud.google.com/gke-nodepool`로 조회해 테이블을 갱신한다.
